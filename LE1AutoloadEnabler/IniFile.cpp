...
std::wstring IniFile::readValue(const std::wstring& section, const std::wstring& key) const
{
	...
	TCHAR buff[buffSize];
	GetPrivateProfileStringW(section.c_str(), key.c_str(), L"", buff, buffSize, iniPath.c_str());
	return std::wstring(buff);
}

void IniFile::removeKey(const std::wstring& section, const std::wstring& key) const
{
	WritePrivateProfileStringW(section.c_str(), key.c_str(), nullptr, iniPath.c_str());
}
